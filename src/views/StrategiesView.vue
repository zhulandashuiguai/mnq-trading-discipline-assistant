...
function showToast(message: string) { toast.value = message; if (toastTimer) window.clearTimeout(toastTimer); toastTimer = window.setTimeout(() => { toast.value = '' }, 2400) }
function select(id: string) { selectedId.value = id }
async function createStrategy() { const strategy = makeStrategy(); await store.saveStrategy(strategy); selectedId.value = strategy.id; draft.value = clone(strategy) }
...
async function saveStrategy() { draft.value.name = draft.value.name.trim() || '未命名策略'; draft.value.entryRules = draft.value.entryRules.map((rule) => rule.trim()).filter(Boolean); draft.value.exitRules = draft.value.exitRules.map((rule) => rule.trim()).filter(Boolean); await store.saveStrategy(draft.value); selectedId.value = draft.value.id; showToast('策略已保存') }
async function moveStrategy(id: string, direction: -1 | 1) { await store.moveStrategy(id, direction); showToast('策略顺序已保存') }
async function removeStrategy(id = selected.value?.id) { const strategy = store.data.strategies.find((item) => item.id === id); if (!strategy || store.data.strategies.length <= 1) return; if (!window.confirm(`删除“${strategy.name}”？`)) return; await store.deleteStrategy(strategy.id); if (selectedId.value === strategy.id) selectedId.value = ''; showToast('策略已删除') }
async function saveSettings() { settingsDraft.value.maxTrades = Math.max(1, Number(settingsDraft.value.maxTrades)); settingsDraft.value.targetProfit = Math.max(1, Number(settingsDraft.value.targetProfit)); settingsDraft.value.maxLoss = Math.max(1, Number(settingsDraft.value.maxLoss)); settingsDraft.value.maxConsecutiveLosses = Math.max(1, Number(settingsDraft.value.maxConsecutiveLosses)); await store.updateSettings(settingsDraft.value); showToast('风险设置已保存') }
</script>
...
<template>
  ...
  <section class="two-column">
    ...
    <section class="panel strategy-editor">
      ...
      <div class="button-row"><button class="primary" @click="saveStrategy">保存策略</button><button class="danger-button" :disabled="store.data.strategies.length <= 1" @click="removeStrategy()">删除策略</button></div>
    </section>
  </section>
  ...
  <section id="risk" class="panel risk-settings">
    ...
    <div class="settings-grid"><label>每日最多交易次数<input v-model.number="settingsDraft.maxTrades" type="number" min="1" /></label><label>日目标盈利（美元）<input v-model.number="settingsDraft.targetProfit" type="number" min="1" /></label><label>日最大亏损额（美元）<input v-model.number="settingsDraft.maxLoss" type="number" min="1" /></label><label>最大连续亏损笔数<input v-model.number="settingsDraft.maxConsecutiveLosses" type="number" min="1" /></label></div>
    <div class="button-row"><button class="primary" @click="saveSettings">保存风险设置</button></div>
  </section>
</template>
